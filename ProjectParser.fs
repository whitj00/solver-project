...
let funCall op =
    inParens
        (pright (pstr (op + " ")) (pmany2sep expr pws1))
    <!> "funCall"
...
let pApplication =
    inParens
        (pseq
            pFunctionName
            (pmany0 (pright pws1 expr))
            (fun (c,d) -> Application(c,d)))
    <!> "pApplication"
...
let manyExpr =
    pleft
        (pseq expr (pmany0 (pright pws1 expr)) (fun (x,xs) -> Program(x::xs)))
        pws0
    <!> "manyExpr"
